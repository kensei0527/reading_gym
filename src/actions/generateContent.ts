...
export async function generateContentAction(profile: Tables<'profiles'>, duration: number): Promise<GeneratedContent> {
  ...
  try {
    ...
    const prompt = `
      ...
      2. 作成する英文は出来るだけ時事的な要素を含ませて、ユーザーが日々学習するうえで飽きの来ない内容の文章にしてください。そして特に浅い文章にならないようにトピックを絞るなどして気を付けてください。
      3. 生成した英文の内容に基づき、以下の指示に従って問題を作成してください。
         ${questionTypeInstruction}
      4. 全ての問題に、詳しい「explanation」（解説）を日本語で付けてください。
      5. 英文全体から、学習者が覚えるべき重要単語やイディオムを5つ選び、「vocabulary」リストを作成してください。リストの各要素は、必ず "word", "definition", "example" の3つのキーを持つオブジェクトにしてください。
        - "word": その単語やイディオム
        - "definition": その単語の簡単な英語での定義
        - "example": 本文中でのその単語を含んだ完全な一文
      6. 以下のJSON形式のスキーマに厳密に従って、応答を生成してください。特に "type" フィールドを各問題に正しく含めてください。
